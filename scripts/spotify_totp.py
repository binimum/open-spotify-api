...
# Ensure we write files to the same directory as the script if running with --all, or handle relative paths better.
# However, the user's request didn't strictly specify path handling, so I'll trust the default behavior but maybe add a little robustness if I can.
# Actually, I'll keep it exactly as provided to minimize risk of breaking it, but I will make sure the Node script calls it from the right CWD or handles the output files correctly.
...
def summarise(caps: List[Dict[str, Any]], mode=None):
    ...
    if mode is None:
        ...
        try:
            # Modified to use absolute paths or current directory explicitly
